...
export async function GET() {
  try {
    const res = await fetch('http://ahmadkoshah-api.htufolio.com/instructor?departmentName=COMPUTER_SCIENCE', {
      cache: 'no-store',
    });
...
export async function POST(request: Request) {
    try {
      ...
      const res = await fetch('http://ahmadkoshah-api.htufolio.com/instructor', {
        method: 'POST',
        headers: {
